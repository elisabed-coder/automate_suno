...
def login_to_account(sb):
    ...
    sb.type('input[type="email"]', 'email') # enter registered email
    sb.click('button:contains("Next")')
    sb.sleep(random.uniform(1, 4))

    sb.type('input[type="password"]', 'password') # enter real email
    sb.click('button:contains("Next")')
...
def enter_prompt_and_create(sb, prompt_text):
    ...
    create_button_xpath = '//*[@id="main-container"]/div[1]/div/div[1]/div/div[4]/div[2]/button'
...
def save_to_db(title, prompt, url, rating):
    ...
    c = conn.cursor()
    c.execute("INSERT INTO songs (title, prompt, url, rating) VALUES (?, ?, ?, ?)", (title, prompt, url, rating))
    conn.commit()
    conn.close()
...
def get_rating_after_listening(sb):
    try:
        ...
        sb.execute_script('window.ratingInput = window.prompt("➡️ Rate the song (1-5):");')
...
with SB(uc=True, undetected=True, incognito=True, locale="en") as sb:
    try:
        ...
        enter_prompt_and_create(sb, "A distant sky seeps misty thoughts… Blowing wind, blowing wind, blowing wind!")
